...
import 'package:vezeeta/features/search/logic/search_bloc.dart';
...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Future<void> _loadRecentSearches() async {
    setState(() {
      _recentSearches = CacheHelper.getStringList(_prefKey) ?? [];
    });
  }

  Future<void> _saveRecentSearch(String query) async {
    if (query.trim().isEmpty) return;
    final list = CacheHelper.getStringList(_prefKey) ?? [];
    list.remove(query);
    list.insert(0, query);
    if (list.length > _maxRecent) list.removeLast();
    await CacheHelper.setStringList(_prefKey, list);
  ...
  Future<void> _clearAllRecentSearches() async {
    await CacheHelper.remove(_prefKey);
